...
export const PhoneChatApp = memo(function PhoneChatApp({ onClose, initialSessionId, onSessionChange, sharePayload, onShareDone }: PhoneChatAppProps) {
    ...
    const handleSelectContact = (sess: ChatSession | null) => {
        if (sharePayload && sess) { /* ... 省略分享逻辑，与原版一致 ... */ }
        setActiveMascot(false);
    ...
    const handleMenuItemClick = (action: "group_chat" | "add_friend" | "scan") => {
        ...
        if (action === "add_friend") {
            ...
            addContactReturnSessionRef.current = activeSessionIdRef.current;
            setActiveTab("contacts"); setActiveSession(null); setActiveMascot(false);
        } else if (action === "scan") { alert("扫一扫功能开发中~"); }
    };
                ...
                {activeTab === "messages" && (
                    <div className="relative flex-1 flex flex-col overflow-hidden">
                                    ...
                                    {showAddMenu && (
                                        <div className="absolute top-[calc(100%+8px)] right-0 bg-white rounded-xl shadow-[0_4px_16px_rgba(0,0,0,0.12)] py-1 w-[140px] z-50 border border-[#f0f0f0]">
                                            <button onClick={() => handleMenuItemClick("group_chat")} className="flex items-center gap-3 w-full px-4 py-3 hover:bg-[#f5f5f5] text-[15px] text-[#333]">
                                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" strokeWidth="2"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg> 发起群聊
                                            </button>
                                            <button onClick={() => handleMenuItemClick("add_friend")} className="flex items-center gap-3 w-full px-4 py-3 hover:bg-[#f5f5f5] text-[15px] text-[#333]">
                                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" strokeWidth="2"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/><line x1="22" y1="11" x2="22" y2="17"/><line x1="19" y1="14" x2="25" y2="14"/></svg> 添加朋友
                                            </button>
                                            <button onClick={() => handleMenuItemClick("scan")} className="flex items-center gap-3 w-full px-4 py-3 hover:bg-[#f5f5f5] text-[15px] text-[#333] border-t border-[#f5f5f5]">
                                                <svg width="18" height="18" viewBox="0 0 24 24" fill="none" strokeWidth="2"><rect x="2" y="2" width="8" height="8" rx="1"/><rect x="14" y="2" width="8" height="8" rx="1"/><rect x="2" y="14" width="8" height="8" rx="1"/><rect x="14" y="14" width="8" height="8" rx="1"/></svg> 扫一扫
                                            </button>
                                        </div>
                                    )}
                                </div>
                        ...
                        <div className="flex-1 overflow-y-auto bg-white pt-[70px]">
                            ...
                            {/* 【重点】第二项起：渲染所有实际存在的聊天角色和群聊，支持未来新增 */}

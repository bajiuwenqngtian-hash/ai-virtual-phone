    ...
    return (
        <div className="chat-app absolute inset-0 flex flex-col overflow-hidden z-10 bg-[#FFFFFF] font-sans">
            ...
            <div className="chat-main-content relative flex-1 flex flex-col overflow-hidden" {...(activeSession || activeMascot ? { "data-covered-by-room": "" } : {})}>
                ...
                {/* 【核心修复】消息列表页：通过剥离 DOM 结构完美隐藏原生头部 */}
                {activeTab === "messages" && (
                    <div className="relative flex-1 flex flex-col overflow-hidden">
                        
                        {/* 1. 底层滚动容器（包含列表） */}
                        <div className="absolute inset-0 overflow-y-auto pt-[70px] pb-2 chat-list-wrapper">
                            {/* 终极隐藏魔法 CSS */}
                            <style>{`
                                /* 剥离 ChatMessageList 的外层包裹，使其子元素直接成为 chat-list-wrapper 的子元素 */
                                .chat-list-wrapper > div { display: contents !important; }
                                /* 完美隐藏原生前4个元素（返回、头像、Chats、搜索、分类） */
                                .chat-list-wrapper > :nth-child(-n+4) { display: none !important; }
                                /* 完美隐藏 AI 助手右下角的绿点 */
                        ...
                        {/* 2. 微信覆盖层：不透明背景+悬浮定位，完美遮盖原生顶部 */}
                        <div className="absolute top-0 left-0 right-0 h-[70px] z-30 bg-[#EDEDED] flex items-center justify-between border-b border-[#E5E5E5] px-4 pt-[max(env(safe-area-inset-top,12px),12px)] pb-2">
                            {/* 隐形退出键（点左侧空白处退出） */}
